        ...
        <div class="row align-items-center g-4">
            <div class="col-lg-7">
                ...
                <form method="GET" class="d-flex gap-2 position-relative" action="{{ route('home') }}" autocomplete="off" id="searchForm">
                    <input id="searchInput" type="text" name="q" value="{{ $search }}" class="form-control form-control-lg" placeholder="Tìm món ăn, cửa hàng...">
                    <button class="btn btn-dark btn-lg" type="submit"><i class="fa-solid fa-magnifying-glass me-2"></i>Tìm</button>
                    <div id="suggestBox" class="position-absolute w-100 mt-2 bg-white rounded-3 border shadow-sm d-none" style="top:100%; left:0; z-index:1050;">
                ...
                <div class="mt-3 d-flex flex-wrap gap-2">
                    ...
                    @foreach($categories as $cat)
                        <a href="{{ route('home', ['category' => $cat->slug, 'q' => $search]) }}" class="category-chip {{ $categorySlug === $cat->slug ? 'bg-dark text-white' : '' }}">{{ $cat->name }}</a>
                    @endforeach
                </div>
            </div>
...
<div class="container">
                ...
                <a href="{{ route('categories.show', $cat->slug) }}" class="text-decoration-none">
                    <div class="p-3 bg-white border rounded-3 h-100 d-flex align-items-center justify-content-between">
                        <div>
                            ...
                            <div class="small text-muted">{{ $cat->active_products_count ?? $cat->products()->where('is_active',1)->count() }} món</div>
                        </div>
    ...
    <!-- Banner ưu đãi -->
    <div class="mb-4">
        <div class="p-4 p-md-5 rounded-4" style="background:linear-gradient(135deg,#ffedd5,#fde68a); border:1px solid #ffe4b5;">
...
<script>
    (function(){
            ...
            timer = setTimeout(async () => {
                ...
                list.innerHTML = data.map(item => `
                    <a class="list-group-item list-group-item-action d-flex align-items-center gap-3" href="${item.url}">
                        <img src="${item.image}" alt="${item.name}" style="width:48px;height:48px;object-fit:cover;border-radius:8px;border:1px solid #eee">
        ...
        // Ngăn click trên input/suggest dẫn đến kích hoạt link card bên dưới
